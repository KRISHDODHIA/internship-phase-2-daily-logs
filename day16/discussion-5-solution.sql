...
> select max(items), min(items), avg(items)
    -> from (select count(ordernumber) as items from orderdetails group by ordernumber) as derived_table;
...
> select ordernumber, total from
    -> (select od.ordernumber, sum(od.priceeach*od.quantityordered) as total
...
> select productcode , sum(quantityordered*priceeach) as sales from orderdetails inner join orders using(ordernumber)
...
> select productname, sales from (select productcode , sum(quantityordered*priceeach) as sales from orderdetails inner join orders using(ordernumber)
    ...
    -> group by productcode
    -> order by sales desc limit 5)
    -> top5products2003 join products using(productcode);
...
> select customerNumber,floor(sum(quantityOrdered*priceEach))
    -> as sales,(case when (sum(quantityOrdered*priceEach)) > 100000 then 'Platinum'
    -> when (sum(quantityOrdered*priceEach)) between 10000 and 100000 then 'Gold'
    -> when (sum(quantityOrdered*priceEach)) < 10000 then 'Silver' end) as customerGroup
    -> from orderdetails inner join orders using(orderNumber) where year(shippedDate)=2003 group by customerNumber order by customerNumber;
...
> select customerGroup,count(customerGroup) as groupCount
    -> from (select customerNumber,floor(sum(quantityOrdered*priceEach))
    -> as sales,(case when (sum(quantityOrdered*priceEach)) > 100000 then 'Platinum'
    -> when (sum(quantityOrdered*priceEach)) between 10000 and 100000 then 'Gold'
    -> when (sum(quantityOrdered*priceEach)) < 10000 then 'Silver' end) as customerGroup
    ...
    -> as derived_table1 group by customerGroup;
